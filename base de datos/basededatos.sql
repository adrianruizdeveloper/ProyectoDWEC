...
CREATE TABLE IF NOT EXISTS `peliculas`.`contrasenas` (
  ...
  `contrasena_contra` VARCHAR(100) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `peliculas`.`usuarios` (
  ...
  `idcontrasena_usu` INT NOT NULL,
  PRIMARY KEY (`id_usu`, `rol_usu`, `idcontrasena_usu`),
  CONSTRAINT `fk_usuarios_tipos_roles1`
    FOREIGN KEY (`rol_usu`)
    REFERENCES `peliculas`.`tipos_roles` (`id_rol`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_usuarios_contrasenas1`
    FOREIGN KEY (`idcontrasena_usu`)
    REFERENCES `peliculas`.`contrasenas` (`id_contrasena`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `peliculas`.`peliculas` (
  ...
  `restriccion_peli` INT NULL,
  `categoria1_peli` INT NULL,
